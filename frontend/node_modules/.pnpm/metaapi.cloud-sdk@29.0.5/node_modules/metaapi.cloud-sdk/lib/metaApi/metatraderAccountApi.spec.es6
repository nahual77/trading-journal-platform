...
describe('MetatraderAccountApi', () => {
  ...
  beforeEach(() => {
    ...
    connectionRegistry = sandbox.createStubInstance(ConnectionRegistry);
  ...
  /**
   * @test {MetatraderAccountApi#getAccountsWithInfiniteScrollPagination}
   */
  it('should retrieve MT accounts with infinite scroll pagination', async () => {
    const apiVersion = '1';
    sandbox.stub(client, 'getAccounts').resolves([{_id: 'id'}]);
    let accounts = await api.getAccountsWithInfiniteScrollPagination({provisioningProfileId: 'profileId'});
    accounts.map(a => a.id).should.match(['id']);
    accounts.forEach(a => (a instanceof MetatraderAccount).should.be.true());
    sinon.assert.calledWithMatch(client.getAccounts, {provisioningProfileId: 'profileId'}, apiVersion);
  });

  /**
   * @test {MetatraderAccountApi#getAccountsWithClassicPagination}
   */
  it('should retrieve MT accounts with classic pagination', async () => {
    const apiVersion = '2';
    sandbox.stub(client, 'getAccounts').resolves({count: 10, items: [{_id: 'id'}]});
    let accounts = await api.getAccountsWithClassicPagination({provisioningProfileId: 'profileId', limit: 1});
    accounts.items.map(a => a.id).should.match(['id']);
    accounts.items.forEach(a => (a instanceof MetatraderAccount).should.be.true());
    sinon.assert.calledWithMatch(client.getAccounts, {provisioningProfileId: 'profileId', limit: 1}, apiVersion);
  });
  ...
  it('should remove MT account', async () => {
    ...
    sinon.assert.calledWith(connectionRegistry.closeAllInstances, 'id');
