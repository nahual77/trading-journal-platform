...
import {MetatraderSymbolSpecification} from './metaApiWebsocket.client.schemas';
...
class MetaApiWebsocketClient {
  ...
  private _minRetryDelayInSeconds: any;
  private _maxRetryDelayInSeconds: any;
  ...
  onAccountDeleted(accountId) {
    ...
    if (masterAccountId) {
      if (masterAccountId === accountId) {
        ...
        const replicas = Object.values<any>(regionData);
   ...
   */
  queuePacket(socket: MetaApiWebsocketClient.SocketInstance, packet) {
    const accountId = packet.accountId;
    const packets = this._packetOrderer.restoreOrder<RestoredOrderPacket>({
    ...
    if (this._sequentialEventProcessing && packet.sequenceNumber !== undefined) {
      const events = packets.map((item: RestoredOrderPacket) => () => (
        this._processSynchronizationPacket(item.socket, item.packet)
      ));
      ...
      packets.forEach((item: RestoredOrderPacket) => this._processSynchronizationPacket(item.socket, item.packet));
    }
  }
   ...
   */
  //eslint-disable-next-line complexity, max-statements
  async rpcRequest(accountId: string, request, timeoutInSeconds?): Promise<any> {
  ...
  private _makeRequest(accountId, instanceNumber, request, timeoutInSeconds) {
  ...
  // eslint-disable-next-line complexity,max-statements
  private async _processSynchronizationPacket(socket: MetaApiWebsocketClient.SocketInstance, data) {
    try {
      ...
      let sourceLabel = _.compact([instanceIndex, socket.clientApiHostname]).join(':');
        ...
        if (this._connectedHosts[instanceId]) {
          ...
          if (isOnlyActiveInstance()) {
            for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
              await this._processEvent(
                ...
                `${primaryAccountId}:${sourceLabel}:onDisconnected`);
            }
          }
          ...
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onStreamClosed`);
          }
      ...
      if (data.type === 'authenticated') {
        ...
        if (!data.sessionId || socketInstance && data.sessionId === socketInstance.sessionId) {
          ...
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onConnected`);
          }
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          await this._processEvent(
            ...
            `${primaryAccountId}:${sourceLabel}:onSynchronizationStarted`);
        }
          ...
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            try {
              await this._processEvent(
                ...
                `${primaryAccountId}:${sourceLabel}:onAccountInformationUpdated`, true);
              ...
              if (this._synchronizationFlags[data.synchronizationId] &&
                ...
                await this._processEvent(
                  ...
                  `${primaryAccountId}:${sourceLabel}:onPositionsSynchronized`, true);
                // eslint-disable-next-line max-depth
                if (!this._synchronizationFlags[data.synchronizationId].ordersUpdated) {
                  await this._processEvent(
                    ...
                    `${primaryAccountId}:${sourceLabel}:onPendingOrdersSynchronized`, true);
                }
              }
            } catch (err) {
              this._logger.error(`${primaryAccountId}:${sourceLabel}: Failed to notify listener ` +
        ...
        for (let deal of (data.deals || [])) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onDealAdded`);
          }
        }
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          try {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPendingOrdersReplaced`, true);
            await this._processEvent(
              () => listener.onPendingOrdersSynchronized(instanceIndex, data.synchronizationId),
              `${primaryAccountId}:${sourceLabel}:onPendingOrdersSynchronized`, true);
          } catch (err) {
        ...
        for (let historyOrder of (data.historyOrders || [])) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onHistoryOrderAdded`);
          }
        }
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          try {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPositionsReplaced`, true);
            await this._processEvent(
              () => listener.onPositionsSynchronized(instanceIndex, data.synchronizationId),
              `${primaryAccountId}:${sourceLabel}:onPositionsSynchronized`, true);
            if (this._synchronizationFlags[data.synchronizationId] &&
        ...
        this._updateEvents[instanceId]?.push({socket, data});
        if (data.accountInformation) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              () => listener.onAccountInformationUpdated(instanceIndex, data.accountInformation),
              `${primaryAccountId}:${sourceLabel}:onAccountInformationUpdated`);
          }
        }
        ...
        if (updatedPositions.length || removedPositionIds.length) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPositionsUpdated`);
          }
        }
        for (let position of updatedPositions) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPositionUpdated`);
          }
        }
        for (let positionId of removedPositionIds) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPositionRemoved`);
          }
        }
        ...
        if (updatedOrders.length || completedOrderIds.length) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPendingOrdersUpdated`);
          }
        }
        for (let order of updatedOrders) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPendingOrderUpdated`);
          }
        }
        for (let orderId of completedOrderIds) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onPendingOrderCompleted`);
          }
        }
        for (let historyOrder of (data.historyOrders || [])) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
        ...
        if (data.timestamps) {
          ...
          for (let listener of this._latencyListeners || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onUpdate`);
          }
        }
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          ...
          await this._processEvent(
            ...
            `${primaryAccountId}:${sourceLabel}:onDealsSynchronized`);
        }
        if (this._updateEvents[instanceId]) {
          let eventCalls = this._updateEvents[instanceId].map(event => (
            () => this._processSynchronizationPacket(event.socket, event.data)
          ));
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          await this._processEvent(
            ...
            `${primaryAccountId}:${sourceLabel}:onHistoryOrdersSynchronized`);
        }
        ...
        if (!this._connectedHosts[instanceId]) {
          if (this._statusTimers[instanceId] && data.authenticated && 
            ...
            // eslint-disable-next-line no-console
            this._logger.info(
              'it seems like we are not connected to a running API server yet, retrying subscription for account' +
              _.compact([instanceId, socket.clientApiHostname]).join(':')
            );
          ...
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onBrokerConnectionStatusChanged`);
          }
          if (data.healthStatus) {
            ...
            for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
              await this._processEvent(
                ...
                `${primaryAccountId}:${sourceLabel}:onHealthStatus`);
            }
          }
        }
      } else if (data.type === 'downgradeSubscription') {
        this._logger.info(`${primaryAccountId}:${sourceLabel}: Market data subscriptions for symbol ` +
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          await this._processEvent(
            ...
            `${primaryAccountId}:${sourceLabel}:onSubscriptionDowngraded`);
        }
        ...
        for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
          await this._processEvent(
            () => listener.onSymbolSpecificationsUpdated(instanceIndex, data.specifications || [],
              data.removedSymbols || []), `${primaryAccountId}:${sourceLabel}:onSymbolSpecificationsUpdated`);
        }
        for (let specification of (data.specifications || [])) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              () => listener.onSymbolSpecificationUpdated(instanceIndex, specification),
              `${primaryAccountId}:${sourceLabel}:onSymbolSpecificationUpdated`);
          }
        }
        for (let removedSymbol of (data.removedSymbols || [])) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onSymbolSpecificationRemoved`);
          }
        }
        ...
        for (let price of prices) {
          for (let listener of this._synchronizationListeners[primaryAccountId] || []) {
            await this._processEvent(
              ...
              `${primaryAccountId}:${sourceLabel}:onSymbolPriceUpdated`);
          }
        }
        for (let price of prices) {
          if (price.timestamps) {
            ...
            for (let listener of this._latencyListeners || []) {
              await this._processEvent(
                ...
                `${primaryAccountId}:${sourceLabel}:onSymbolPrice`);
            }
          }
        }
      }
  ...
  // eslint-disable-next-line complexity
  private async _getServerUrl(instanceNumber: number, socketInstanceIndex, region) {
    if (this._url) {
      return this._url;
    }
  ...
  private _clearAccountCacheJob() {
    ...
    Object.keys(this._regionsByAccounts).forEach(replicaId => {
      ...
      if (data && data.connections === 0 && date - data.lastUsed > 2 * 60 * 60 * 1000) {
        ...
        const replicas = Object.values<any>(this._accountReplicas[primaryAccountId] || {});
        replicas.forEach(replica => {
          delete this._accountsByReplicaId[replica];
          delete this._regionsByAccounts[replica];
        });
...
namespace MetaApiWebsocketClient {

  /** Account replicas by region (including primary replica) by primary ID */
  export type AccountReplicas = {[primaryId: string]: MetatraderAccount.AccountsByRegion};
   ...
   */
  export type AccountsByReplica = {[replicaId: string]: string};
...
/**
 * Regions by accounts
 * - keys are replica or primary account ID
 */
type RegionsByAccounts = {[accountId: string]: {
  region: string;
  connections: number;
  lastUsed: number;
}};

type SynchronizationFlag = {
  accountId: string,
  instanceNumber: number;
...
type SocketInstances = {[region: string]: {[instanceNumber: number]: MetaApiWebsocketClient.SocketInstance[]}};

type Packet = {
  socket: MetaApiWebsocketClient.SocketInstance;
  data: any;
};

type RestoredOrderPacket = PacketOrderer.Packet & {
  socket: MetaApiWebsocketClient.SocketInstance;
  packet: any;
};
